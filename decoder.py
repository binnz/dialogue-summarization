...
class Decoder(nn.Module):
    ...
    def forward(self, target_features, target_mask, encode_features,
                         utterance_mask, token_features, token_mask, coverage, step):
        ...
        utters_attn_dist = []
        utters_outout = []
        utters_coverage = torch.zeros_like(token_mask).contiguous().float()

        for u_index in range(utter_length):
            output, attn_dist, coverage = self.token_attention(local_dec_feature, token_features[u_index], token_mask[u_index], utters_coverage[u_index])
            utters_attn_dist.append(attn_dist)
            utters_outout.append(output)
            utters_coverage[u_index] = coverage
        ...
        target_attn_dist = torch.index_select(utters_attn_dist, 0, utter_index)
        target_out_put = torch.index_select(utters_outout, 0, utter_index)
...
class LocalAttention(nn.Module):
    ...
    def forward(self, target_t, encode_features, source_mask, coverage):
        b, t_k, e_dim = list(encode_features.size())
